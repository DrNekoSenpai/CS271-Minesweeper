...
for step in range(1000):
    ...
    for i in range(num_envs):
        ...
        if done[i]:
            ...
            # Check win condition (same as training)
            if rewards[i] == 100.0:
                recent_wins.append(1)
                print(f"✓ Win detected! Env {i}, step {step}, terminal_reward={rewards[i]:.1f}, episode_total={episode_rewards[i]:.1f}")
            elif rewards[i] == -100.0:
                recent_wins.append(0)
            else:
                recent_wins.append(0)
                print(f"✗ Terminal but reward is {rewards[i]:.1f}, not ±100.0! Env {i}, step {step}")
...
print(f"  +100.0 count: {sum(1 for r in terminal_rewards_seen if r == 100.0)}")
print(f"  -100.0 count: {sum(1 for r in terminal_rewards_seen if r == -100.0)}")
print(f"  Other count: {sum(1 for r in terminal_rewards_seen if r != 100.0 and r != -100.0)}")

if any(r != 100.0 and r != -100.0 for r in terminal_rewards_seen):
    print(f"\n[WARNING] BUG FOUND: Terminal rewards that aren't +/-100.0!")
    weird_rewards = [r for r in terminal_rewards_seen if r != 100.0 and r != -100.0]
    print(f"  Weird rewards: {weird_rewards[:10]}")
else:
    print(f"\n[PASS] All terminal rewards are correctly +/-100.0")
